...
import { ArrowLeft, Plus, Mail, Clock } from "lucide-react";
...
export default function ProcessDetails() {
  ...
  const handleUpdateStatus = (candidateId: string, status: CandidateStatus, description: string) => {
    setCandidates(candidates.map(c => 
      c.id === candidateId 
        ? { ...c, status, statusDescription: description, updatedAt: new Date().toISOString() }
        : c
    ));
  };
                ...
                <CardContent className="space-y-4">
                  {candidate.statusDescription && (
                    <div className="rounded-lg bg-muted p-3">
                      <p className="text-sm">{candidate.statusDescription}</p>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <Clock className="h-3 w-3" />
                      Last updated: {format(new Date(candidate.updatedAt), 'MMM dd, yyyy HH:mm')}
                    </div>
                    <Button 
                      variant="outline" 
                      size="sm"
